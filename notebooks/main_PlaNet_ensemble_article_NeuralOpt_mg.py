...
tRRconv = tf.convert_to_tensor(RR_in, dtype=(DTYPE))

tRRconv = tf.expand_dims(tf.expand_dims(tRRconv,axis = 0),axis = -1)

Laplace_kernel = tf.convert_to_tensor(Laplace_kernel, dtype=(DTYPE))
Df_dr_kernel = tf.convert_to_tensor(Df_dr_kernel, dtype=(DTYPE))
tLaplace_kernel = tf.expand_dims(tf.expand_dims(Laplace_kernel[::-1, ::-1],axis = -1),axis = -1)
tDf_dr_kernel = tf.expand_dims(tf.expand_dims(Df_dr_kernel[::-1, ::-1],axis = -1),axis = -1)
tGaussian_kernel = tf.expand_dims(tf.expand_dims(Gaussian_kernel[::-1, ::-1],axis = -1),axis = -1)


# routines to compute GS equation via convolution
def fun_GSoperator_NN_conv_batch(f):
    # Compute GS operator via convolution
    Lpsi = tf.nn.conv2d(f,tLaplace_kernel,strides=[1, 1, 1, 1],padding='VALID')
    Dpsi_dr = tf.nn.conv2d(f,tDf_dr_kernel,strides=[1, 1, 1, 1],padding='VALID')
    Dpsi_dr = tf.math.divide(Dpsi_dr,tRRconv)
...
def fun_GSoperator_NN_conv_smooth_batch(f):
    ...
    GS_ope = Lpsi - Dpsi_dr
    GS_ope = GS_ope*alfa/(hr**2*hz**2)
    GS_ope = tf.nn.conv2d(GS_ope,tGaussian_kernel,strides=[1, 1, 1, 1],padding='SAME')
