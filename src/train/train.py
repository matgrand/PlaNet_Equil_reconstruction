...
class Trainer():
    ...
    def InitCheckpointManager(self):
        ...
        self.manager = tf.train.CheckpointManager(self.ckpt, self.config['train']['ckpt_path'], max_to_keep=4)
        if self.config['train']['from_ckpt']:
            self.ckpt.restore(self.manager.latest_checkpoint)
            if self.manager.latest_checkpoint:
                print("Restored from {}".format(self.manager.latest_checkpoint))
                with open(self.config['train']['ckpt_path'] + 'history_ckpt.pkl','rb') as f: self.history = pickle.load(f)
            else: print("No checkpoint found, initializing from scratch."), self.init_history()
        else: print("Initializing from scratch."), self.init_history()
            
    def init_history(self): self.history = {'loss':[],'loss_iter':[],'loss_Psi':[],'loss_GSO':[]}

    def ckeckpoint_here(self):
        save_path = self.manager.save()
        print("Saved checkpoint for step {}: {}".format(int(self.ckpt.step), save_path))
        with open(self.config['train']['ckpt_path'] + 'history_ckpt.pkl','wb') as f: pickle.dump(self.history, f)
        
    def loss_fun_all(self,y_ds,predictions,RHS_in_ds,RR_ds,ZZ_ds,Laplace_kernel_ds,Df_dr_kernel_ds,learning_rate_PDE=1,learning_rate_Grad=1):
        loss_Psi = self.loss_fun_MSE(y_ds, predictions)
        if self.config['train']['is_physics_informed']:
            loss_GSO = self.loss_fun_PDE_adaptive(predictions,RHS_in_ds,Laplace_kernel_ds,Df_dr_kernel_ds,RR_ds,ZZ_ds)
            return loss_Psi + loss_GSO
        else: return loss_Psi
    ...
    def train_step(self,train_ds_i,learning_rate_PDE=1,learning_rate_Grad=1):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = self.loss_fun_all(y_ds,predictions,RHS_in_ds,RR_ds,ZZ_ds,L_ker_ds,Df_ker_ds,learning_rate_PDE,learning_rate_Grad)
        gradients = tape.gradient(loss, self.model.trainable_variables)
        self.optimizer.apply_gradients(zip(gradients, self.model.trainable_variables))
        return loss
    
    @staticmethod
    def loss_fun_MSE(y_ds, predictions): return tf.reduce_mean(tf.square(y_ds - tf.squeeze(predictions)))
    
    def loss_fun_PDE_adaptive(self, predictions, RHS_in_ds, Laplace_kernel_ds, Df_dr_kernel_ds, RR_ds, ZZ_ds):
        GS_ope_ref = RHS_in_ds
        GS_ope_ds = fun_GSoperator_NN_conv_smooth_batch_adaptive(predictions, Laplace_kernel_ds, Df_dr_kernel_ds, self.model.Gauss_tensor, RR_ds, ZZ_ds)
    ...
    def run(self):
        ...
        for epoch in tqdm(range(epoch_start,epochs),mininterval=5):
            ...
            loss_GSO = self.loss_fun_PDE_adaptive(preds,RHS_in_ds,L_ker_ds,Df_ker_ds,RR_ds,ZZ_ds).numpy()
            self.history['loss_Psi'].append(loss_Psi)
            self.history['loss_GSO'].append(loss_GSO)
            # Print some statistics
            print('epoch {:d}/{:d}, loss: {:2.2e}, loss Psi: {:2.2e}, loss GSO: {:2.2e}, {:d}s/epoch'.format(epoch+1,epochs,self.history['loss'][-1],loss_Psi,loss_GSO,int(epoch_time)))
            if self.config['train']['logging']:
                for k in ['loss_Psi','loss_GSO']: self.logger.log({k: self.history[k][-1]})
            self.ckpt.step.assign_add(1)
            self.ckeckpoint_here()
        total_training_time = time() - training_time_start
        print('\n--> Total training time {:d}s\n'.format(int(total_training_time)))
        self.save_trained_model()

    def save_trained_model(self):
        if self.config['train']['trained_model']:
            filename = self.config['train']['trained_model']['dir'] + self.config['train']['trained_model']['name']
